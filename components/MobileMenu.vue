...
<template>
   <div
      class="bg-warm-white not-first-of-type:border-warm-black fixed left-0 right-0 top-[4rem] z-10 overflow-hidden border-b-[1.5px] transition-[height] duration-300 ease-in-out md:top-[3.5rem] lg:top-[4.5rem]"
      :class="{
      ...
      <nav role="navigation" class="px-5 py-8 sm:px-8">
         <ul>
            <li v-for="item in menuItems" :key="item.name">
               <a
                  :href="item.href"
                  class="max-w-18 font-poppins text-warm-black group block py-4 pe-3 text-xl font-medium"
         ...
         <div class="mt-8 flex flex-col gap-5 border-t border-stone-200 py-8">
            <div class="text-warm-black nt-poppins text-center text-lg">Follow me!</div>
            <div class="flex justify-center gap-6">
               <a
                  v-for="social in socialLinks"
                  :key="social.label"
                  :href="social.href"
                  class="border-warm-black shadow-warm-black flex items-center justify-center rounded border p-4 transition-colors hover:bg-emerald-400 hover:text-white"
